...
uint8_t registers[WIRED_CONTROLLER_ASUKIAAA_REGISTER_LENGTH];
uint8_t registerIndex = 0;

bool pushed(uint8_t pin) { return digitalRead(pin) == LOW; }
...
int16_t getJoystickHorizontalValue() {
  int16_t v =
      getJoystickValue(JOYSTICK_HORIZONTAL_UPPER, JOYSTICK_HORIZONTAL_MIDDLE);
...
int16_t getJoystickVerticalValue() {
  int16_t v =
      getJoystickValue(JOYSTICK_VERTICAL_UPPER, JOYSTICK_VERTICAL_MIDDLE);
...
void onRequest() {
  if (registerIndex < WIRED_CONTROLLER_ASUKIAAA_REGISTER_LENGTH) {
    Wire.write(&registers[registerIndex],
               WIRED_CONTROLLER_ASUKIAAA_REGISTER_LENGTH - registerIndex);
  } else {
    Wire.write(0);
  }
}
...
void setup() {
  ...
  updateLeds();
  registers[WIRED_CONTROLLER_ASUKIAAA_REGISTER_PROTOCOL_VERSION] =
      PROTOCOL_VERSION;

  Wire.begin(digitalRead(ADDR_JUMPER) == HIGH
                 ? WIRED_CONTROLLER_ASUKIAAA_ADDRESS_DEFAULT
...
void updateJoystickRegisters() {
  ...
  registers[WIRED_CONTROLLER_ASUKIAAA_REGISTER_JOY_VERT_UPPER] =
      joystickVertical >> 8;
  registers[WIRED_CONTROLLER_ASUKIAAA_REGISTER_JOY_VERT_LOWER] =
      joystickVertical & 0xff;
  registers[WIRED_CONTROLLER_ASUKIAAA_REGISTER_JOY_HORI_UPPER] =
      joystickHorizontal >> 8;
  registers[WIRED_CONTROLLER_ASUKIAAA_REGISTER_JOY_HORI_LOWER] =
      joystickHorizontal & 0xff;
}
